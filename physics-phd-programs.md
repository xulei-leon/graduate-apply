# Physics PhD Programs

Direct-entry PhD programs (accepting bachelor's directly) in the US and Singapore, organized by advisor match for dark matter, Bayesian inference, galaxy dynamics, and computational astrophysics research.

## Index table

| # | Country | University | Program | Advisor | Title | Research focus | Direct entry | Match | Contact |
|---|---------|-----------|---------|---------|-------|----------------|-------------|-------|---------|
| 1 | USA | [MIT](physics-phd/MIT_Necib.md) | Physics PhD | Lina Necib | Asst. Prof. | Galactic dynamics, dark matter map of Milky Way, Gaia + ML | Yes | High | Not contacted |
| 2 | USA | [MIT](physics-phd/MIT_Slatyer.md) | Physics PhD | Tracy Slatyer | Professor | Dark matter indirect detection, Bayesian | Yes | Medium | Not contacted |
| 3 | USA | [Stanford](physics-phd/Stanford_Wechsler.md) | Physics PhD | Risa Wechsler | Professor | Galaxy-dark matter connection, SAGA Survey | Yes | High | Not contacted |
| 4 | USA | [Stanford](physics-phd/Berkeley_Seljak.md) | Physics PhD | Uros Seljak | Professor | Bayesian inference, cosmology, ML methods | Yes | High | Not contacted |
| 5 | USA | [UC Berkeley](physics-phd/Berkeley_Ferraro.md) | Physics PhD | Simone Ferraro | Sr. Scientist | Data-driven cosmology, CMB + LSS + ML | Yes | High | Not contacted |
| 6 | USA | [UC Davis](physics-phd/UCDavis_Wetzel.md) | Physics PhD | Andrew Wetzel | Professor | FIRE simulations, DM halo formation | Yes | High | Not contacted |
| 7 | USA | [UCLA](physics-phd/UCLA_Treu.md) | Physics PhD | Tommaso Treu | Professor | Strong lensing, c-M relation constraints | Yes | High | Not contacted |
| 8 | USA | [University of Michigan](physics-phd/Michigan_Valluri.md) | Physics PhD | Monica Valluri | Res. Prof. | Galactic dynamics, DM halos, tidal streams | Yes | High | Not contacted |
| 9 | USA | [University of Pennsylvania](physics-phd/UPenn_Sanderson.md) | Physics PhD | Robyn Sanderson | Assoc. Prof. | Galaxy dynamics, dark matter in Milky Way | Yes | High | Not contacted |
| 10 | USA | [Yale University](physics-phd/Yale_vandenBosch.md) | Physics PhD | Frank van den Bosch | Professor | Halo occupation, DM substructure | Yes | High | Not contacted |
| 11 | USA | [Johns Hopkins](physics-phd/JHU_Wandelt.md) | Physics PhD | Benjamin Wandelt | Professor | Bayesian inference, CMB, astrostatistics | Yes | High | Not contacted |
| 12 | USA | [University of Pittsburgh](physics-phd/Pittsburgh_Zentner.md) | Physics PhD | Andrew Zentner | Professor | Dark matter halos, galaxy clustering, cosmology | Yes | High | Not contacted |
| 13 | USA | [Ohio State](physics-phd/OSU_Weinberg.md) | Physics PhD | David Weinberg | Distinguished Prof. | Galaxy formation, DM on small scales, CCAPP | Yes | High | Not contacted |
| 14 | USA | [UW–Madison](physics-phd/UWMadison_Cranmer.md) | Physics PhD | Kyle Cranmer | Professor | Bayesian inference, ML for physics | Yes | High | Not contacted |
| 15 | USA | [Georgia Tech](physics-phd/GeorgiaTech_Wise.md) | Physics PhD | John Wise | Professor | Computational astrophysics, Enzo code, HPC galaxy simulations | Yes | High | Not contacted |
| 16 | USA | [Georgia Tech](physics-phd/GeorgiaTech_Psaltis.md) | Physics PhD | Dimitrios Psaltis | Professor | Computational astrophysics, ML, AI4Science, black holes | Yes | High | Not contacted |
| 17 | USA | [University of Utah](physics-phd/Utah_Mao.md) | Physics PhD | Yao-Yuan Mao | Asst. Prof. | DM halos, SAGA Survey | Yes | High | Not contacted |
| 18 | USA | [University of Washington](physics-phd/UW_Baryakhtar.md) | Physics PhD | Masha Baryakhtar | Asst. Prof. | Dark matter theory, particle astrophysics | Yes | Medium | Not contacted |
| 19 | USA | [NYU](physics-phd/NYU_Shih.md) | Physics PhD | David Shih | Professor | ML for astrophysics, dark matter | Yes | Medium | Not contacted |
| 20 | USA | [UIUC](physics-phd/UIUC_Trickle.md) | Physics PhD | Tanner Trickle | Asst. Prof. | Dark matter phenomenology | Yes | Medium | Not contacted |
| 21 | Singapore | [NUS](physics-phd/NUS_PhysicsPhD.md) | Physics PhD | TBD | — | No strong match | Yes | Low | Not contacted |
| 22 | Singapore | [NTU](physics-phd/NTU_PhysicsPhD.md) | Physics PhD | TBD | — | No strong match | Yes | Low | Not contacted |

## Geographic distribution

| Region | Count | Universities |
|--------|-------|-------------|
| USA | 20 | MIT (x2), Stanford (x2), UC Berkeley (x2), UC Davis, UCLA, Michigan, UPenn, Yale, Johns Hopkins, Pittsburgh, Ohio State, Georgia Tech (x2), UW–Madison, Utah, UW, NYU, UIUC |
| Singapore | 2 | NUS, NTU |

**Total: 22 targets**

## Summary by match

| Match | Count | Notes |
|-------|-------|-------|
| High | 17 | Strong overlap with computational physics, dark matter, galaxy dynamics, Bayesian inference, computational astrophysics |
| Medium | 3 | Topical overlap but focus differs (indirect detection / particle phenomenology / ML methods) |
| Low | 2 | Singapore programs lack matching faculty |

## Per-program files

See individual `.md` files in the `physics-phd/` folder for full program and advisor details.
